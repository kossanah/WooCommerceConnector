            ...
            if not sales_order:
                if valid_customer_and_product(woocommerce_order):
                    try:
                        ...
                        if error.args and error.args[0] and str(error.args[0]).startswith(b"402"):
                            raise error
                        make_woocommerce_log(title=str(error), status="Error", method="sync_woocommerce_orders", message=frappe.get_traceback(),
                                             request_data=woocommerce_order, exception=True)
...
def valid_customer_and_product(woocommerce_order):
    ...
    warehouse = frappe.get_doc(
        "WooCommerce Config", "WooCommerce Config").warehouse
...
def get_country_name(code):
    coutry_name = ''
    coutry_names = """SELECT `country_name` FROM `tabCountry` WHERE `code` = '{0}'""".format(
        code.lower())
...
def create_sales_order(woocommerce_order, woocommerce_settings, company=None):
    ...
    backup_customer = frappe.get_all("Customer", filters=[
                                     ["woocommerce_customer_id", "=", "Guest of Order-ID: {0}".format(woocommerce_order.get("id"))]], fields=['name'])
    if customer:
    ...
    if not so:
        ...
        tax_rules = frappe.get_all("WooCommerce Tax Rule", filters={
                                   'currency': woocommerce_order.get("currency")}, fields=['tax_rule'])
        if not tax_rules:
        ...
        so = frappe.get_doc({
            ...
            "customer": customer,
            # hard code group, as this was missing since v12
            "customer_group": woocommerce_settings.customer_group,
            ...
            # pull posting date from WooCommerce
...
def get_customer_address_from_order(type, woocommerce_order, customer):
    ...
    address_name = frappe.db.get_value("Address", {"woocommerce_address_id": type, "address_line1": address_record.get(
        "address_1"), "woocommerce_company_name": address_record.get("company") or ''}, "name")
    if not address_name:
...
def get_item_code(woocommerce_item):
    if cint(woocommerce_item.get("variation_id")) > 0:
        ...
        item_code = frappe.db.get_value(
            "Item", {"woocommerce_product_id": woocommerce_item.get("variation_id")}, "item_code")
    else:
        # single
        item_code = frappe.db.get_value(
            "Item", {"woocommerce_product_id": woocommerce_item.get("product_id")}, "item_code")
...
def get_order_taxes(woocommerce_order, woocommerce_settings):
    ...
    for tax in woocommerce_order.get("tax_lines"):
        ...
        name = woocommerce_tax.get("label") or woocommerce_tax.get("name")
...
def get_shipping_account_head(shipping):
    shipping_title = (shipping.get("method_title")).encode(
    ...
    if not shipping_account:
        frappe.throw("Tax Account not specified for woocommerce shipping method  {0}".format(
...
def get_tax_account_head(tax):
    tax_title = (tax.get("name") or tax.get("method_title")).encode(
        "utf-8", errors="ignore").decode("utf-8")
