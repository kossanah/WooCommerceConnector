...
def make_item(warehouse, woocommerce_item, woocommerce_item_list):
    ...
    if has_variants(woocommerce_item):
        ...
        create_item_variants(woocommerce_item, warehouse, attributes,
                             woocommerce_variants_attr_list, woocommerce_item_list)
    ...
    if len(attributes) > 0:
        for attr in attributes:
            if attr["attribute"] == woocommerce_settings.attribute_for_uom:
                uom_match = frappe.get_all("UOM", filters={'uom_name': "{0}".format(
                    attr["attribute_value"])}, fields=['name'])
                if len(uom_match) > 0:
...
def get_categories(woocommerce_item, is_variant=False):
    ...
    if not is_variant:
        ...
        try:
            erpnext_categories = frappe.db.sql("""SELECT `category` FROM `tabItem Product Category` WHERE `parent` = '{item_code}'""".format(
                item_code=woocommerce_item.name), as_list=True)
...
def is_item_exists(item_dict, attributes=None, variant_of=None, woocommerce_item_list=[]):
    ...
    erp_item_match = frappe.get_all("Item",
                                    filters={'woocommerce_product_id': item_dict.get(
    ...
    for attr in woocommerce_item.get('attributes'):
        if not frappe.db.get_value("Item Attribute", attr.get("name"), "name"):
            ...
            for attr_value in attr.get("options"):
                row = new_item_attribute_entry.append(
                    'item_attribute_values', {})
            ...
            if not item_attr.numeric_values:
                ...
                item_attr = set_new_attribute_values(
                    item_attr, attr.get("options"))
...
def add_to_price_list(item, name):
    ...
    rate = item.get("sale_price") or item.get(
        "price") or item.get("item_price") or 0
...
def sync_erpnext_items(price_list, warehouse, woocommerce_item_list):
    ...
    for item in get_erpnext_items(price_list):
        try:
            sync_item_with_woocommerce(item, price_list, warehouse, woocommerce_item_list.get(
...
def get_erpnext_items(price_list):
    ...
    item_from_master = """select name, item_code, item_name, item_group,
        description, woocommerce_description, has_variants, variant_of, stock_uom, image, woocommerce_product_id,stock_keeping_unit,
...
def sync_item_with_woocommerce(item, price_list, warehouse, woocommerce_item=None):
    ...
    if not item.get("woocommerce_product_id") or not erp_item.woocommerce_product_id:
        ...
        try:
            # remove image from item_data dictionary
            item_data.pop("images", None)
            # update the dictionary result from sync_item_image(item) to item_data["images"] list
            # item_data["images"] = []
            # images = sync_item_image(item)
            # if images:
            #     for image in images:
    ...
    if variant_list:
        for variant in variant_list:
            ...
            if erp_varient_item.woocommerce_product_id:  # varient exist in woocommerce let's update only
                r = put_request("products/{0}/variations/{1}".format(
                    erp_item.woocommerce_product_id, erp_varient_item.woocommerce_product_id), variant)
            else:
                woocommerce_variant = post_request(
                    "products/{0}/variations".format(erp_item.woocommerce_product_id), variant)

                erp_varient_item.woocommerce_product_id = woocommerce_variant.get(
                    "id")
                erp_varient_item.woocommerce_variant_id = woocommerce_variant.get(
                    "id")
    ...
    if erp_item.image:
        ...
        if not item_image or datetime.datetime(item_image.date_modified, '%Y-%m-%dT%H:%M:%S') < datetime.datetime(img_details[0], '%Y-%m-%d %H:%M:%S.%f'):
            # to sync image to woocommerce include image id in item_data["images"] list
            # image_info = sync_item_image(erp_item)
...
def item_image_exists(woocommerce_product_id, image_info):
    ...
    for image in get_woocommerce_item_image(woocommerce_product_id):
        if image_info.get("image").get("filename"):
            if cstr(os.path.splitext(image.get("src"))[0].split("/")[-1]) == cstr(os.path.splitext(image_info.get("image").get("filename")))[0]:
                return True
        elif image_info.get("image").get("src"):
            if cstr(os.path.splitext(image.get("src"))[0].split("/")[-1]) == cstr(os.path.splitext(image_info.get("image").get("src")))[0].split("/")[-1]:
                return True
        else:
            return False
...
def update_variant_item(new_item, item_code_list):
    for i, name in enumerate(item_code_list):
        ...
        erp_item.flags.ignore_mandatory = True
        erp_item.woocommerce_product_id = new_item['product']["variants"][i].get(
            "id")
        erp_item.woocommerce_variant_id = new_item['product']["variants"][i].get(
            "id")
        erp_item.save()
...
def get_variant_attributes(item, price_list, warehouse):
    ...
    for i, variant in enumerate(frappe.get_all("Item", filters={"variant_of": item.get("name")},
        ...
        data = (get_price_and_stock_details(
            item_variant, warehouse, price_list))
...
def get_price_and_stock_details(item, warehouse, price_list):
    actual_qty = frappe.db.get_value("Bin", {"item_code": item.get(
        "item_code"), "warehouse": warehouse}, "actual_qty")
...
def update_item_stock_qty(force=False):
    ...
    for item in frappe.get_all("Item", fields=["item_code"], filters={"sync_qty_with_woocommerce": '1', "disabled": ("!=", 1)}):
        try:
            update_item_stock(
                item.item_code, woocommerce_settings, force=force)
        except woocommerceError as e:
            make_woocommerce_log(title="{0}".format(e), status="Error", method="sync_woocommerce_items", message=frappe.get_traceback(),
                                 request_data=item, exception=True)
    ...
    if item.sync_qty_with_woocommerce:
        if not item.woocommerce_product_id:
            ...
            last_sync_datetime = get_datetime(
                woocommerce_settings.last_sync_datetime)
            bin_since_last_sync = frappe.db.sql("""SELECT COUNT(`name`) FROM `tabBin` WHERE `item_code` = '{item_code}' AND `modified` > '{last_sync_datetime}'""".format(
                item_code=item_code, last_sync_datetime=last_sync_datetime), as_list=True)[0][0]
            if bin_since_last_sync > 0 or force != False:
                ...
                if item.woocommerce_product_id and item.variant_of:
                    ...
                    item_data, resource = get_product_update_dict_and_resource(
                        template_item, woocommerce_variant_id=item.woocommerce_product_id, is_variant=True, actual_qty=qty)
                else:
                    # item = single
                    item_data, resource = get_product_update_dict_and_resource(
...
def load_price(item, debug=False):
    ...
    if erp_item_code:
        ...
        if debug:
            print("Updated {0} with {1}".format(
                erp_item_code, item.get('price')))
    return
