...
# This implements functions related to syncing data between ERPNext and WooCommerce.
...
def sync_woocommerce():
    ...
    if timeout < 60:
        timeout = 60
    enqueue("woocommerceconnector.api.sync_woocommerce_resources",
...
def sync_woocommerce_resources():
    ...
    make_woocommerce_log(title="Sync Job Queued", status="Queued",
                         method=frappe.local.form_dict.cmd, message="Sync Job Queued")

    if woocommerce_settings.enable_woocommerce:
        make_woocommerce_log(title="Sync Job Started", status="Started",
                             method=frappe.local.form_dict.cmd, message="Sync Job Started")
        try:
            ...
            sync_products(woocommerce_settings.price_list, woocommerce_settings.warehouse,
                          True if woocommerce_settings.sync_items_from_woocommerce_to_erp == 1 else False)
            sync_customers()
            sync_orders()
            ...
            frappe.db.set_value("WooCommerce Config", None,
                                "last_sync_datetime", sync_start_time)
...
def sync_woocommerce_ids():
    "Enqueue longjob for syncing woocommerce"
    enqueue("woocommerceconnector.sync_products.add_w_id_to_erp",
